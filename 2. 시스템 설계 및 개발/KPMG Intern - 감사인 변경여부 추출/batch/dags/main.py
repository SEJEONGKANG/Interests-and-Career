...
db_connection_url = 'mysql+pymysql://root:password@mysql_db:3306/sr'
...
class Assigned(Base):
    ...
    __table_args__ = (Index("is_assigned_corp_code_assigned_year", "corp_code", "assigned_year"),)
...
def run():
    """Airflow에서 호출하는 어제자 공시 보고서로부터 정보를 추출하여 DB에 적재하는 전체 코드를 실행시키는 함수"""
    ...
    update_target = make_update_target(yesterday,yesterday)
    ...
    print("corp table update 완료")

    push_report_to_db(reports_result, session)
    print("report table update 완료")
